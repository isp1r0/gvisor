...
type Pipe struct {
	...
	// buf is a circular buffer holding the pipe's data. buf is a circular
	// buffer; the first valid byte in buf is at offset off, and the pipe
	// contains size valid bytes. bufBlocks contains two identical
	// safemem.Blocks representing buf; this is used to avoid heap allocation
	// of a safemem.BlockSeq when the buffer wraps around.
	//
...
// to f will be less than count bytes in length, and peekLocked will return
// ErrWouldBlock or EOF if f is otherwise successful.
//
...
func (p *Pipe) peekLocked(count int64, f func(safemem.BlockSeq) (uint64, error)) (int64, error) {
	...
	bs := safemem.BlockSeqFromSlice(p.bufBlocks[:]).DropFirst64(uint64(p.off)).TakeFirst64(uint64(count))
	...
	// If we shortened the read, adjust the returned error appropriately.
	if short {
		if !p.HasWriters() {
			return done, io.EOF
		}
...
func (p *Pipe) writeLocked(count int64, f func(safemem.BlockSeq) (uint64, error)) (int64, error) {
	...
	if newLen, oldCap := p.size+count, int64(len(p.buf)); newLen > oldCap {
		...
		safemem.CopySeq(
			...
			safemem.BlockSeqFromSlice(p.bufBlocks[:]).DropFirst64(uint64(p.off)).TakeFirst64(uint64(p.size)))
	...
	bs := safemem.BlockSeqFromSlice(p.bufBlocks[:]).DropFirst64(uint64(woff)).TakeFirst64(uint64(count))
